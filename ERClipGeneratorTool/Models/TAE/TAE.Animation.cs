    ...
    public class Animation
    {
            ...
            br.StepIn(offset);
            {
                    ...
                    for (int i = 0; i < eventCount; i++)
                    {
                        ...
                        if (i > 0)
                        {
                            ...
                            br.StepIn(eventParameterOffsets[i - 1]);
                            {
                                ...
                                long gapBetweenEventParamOffsets =
                                    eventParameterOffsets[i] - eventParameterOffsets[i - 1];
                                // Subtract to account for the current event's type and offset 
                                Events[i - 1].ReadParameters(br,
                                    (int)(gapBetweenEventParamOffsets - (br.VarintLong ? 16 : 8)));
                            }
                            br.StepOut();
                        }
                    }
                ...
                if (eventCount > 0)
                {
                    ...
                    else
                    {
                        ...
                        br.StepIn(eventParameterOffsets[eventCount - 1]);
                        {
                            ...
                            Events[eventCount - 1].ReadParameters(br,
                                (int)(eventGroupsOffset - eventParameterOffsets[eventCount - 1]));
                        }
                        br.StepOut();
                    }
                }
                ...
                br.StepIn(eventGroupsOffset);
                {
                    for (int i = 0; i < eventGroupCount; i++)
                        EventGroups.Add(
                            new TAE.EventGroup(br, eventHeaderOffsets, format));
                }
                br.StepOut();
        ...
        internal void WriteEventGroupHeaders(BinaryWriterEx bw, int i, TAEFormat format)
        {
            if (EventGroups.Count > 0)
            {
                ...
                {
                    EventGroups[j].indices = Events.Where(ev => ev.Group == EventGroups[j])
                        .Select(ev => Events.IndexOf(ev)).ToList();
        ...
        public abstract class AnimMiniHeader
        {
            ...
            public sealed class Standard : AnimMiniHeader
            {
                ...
                /// <summary>
                /// Whether to allow this animation to be loaded from delayload anibnds such as the c0000_cXXXX.anibnd player throw
                /// anibnds.
                /// </summary>
